...
function PostCard({ $id, title, featuredImage, content }) {
  ...
  return (
    <Link to={`/post/${$id}`} className="block">
      <div className="max-w-sm h-90 p-6 rounded-lg shadow-md bg-white hover:shadow-xl transition-all duration-300 flex flex-col border border-gray-200">
        <img
          src={filePreviewURL}
        ...
        <p className="text-gray-600 text-sm line-clamp-2 mt-2">{parse(snippet)}</p>
      </div>
    </Link>
  );
}
